...
export async function getFx(): Promise<ApiResponse<Record<string, number>>> {
  return fetchJson<ApiResponse<Record<string, number>>>('/api/currency/latest?base_currency=USD', CACHE_TTL_MS, true);
}
...
export async function getPortfolio(userId?: string): Promise<ApiResponse<PortfolioEntry[]>> {
  ...
  return fetchJson<ApiResponse<PortfolioEntry[]>>('/api/portfolio', CACHE_TTL_MS, true, headers, cacheKey);
}
...
export async function getCoinInfo(id: string | number): Promise<CoinInfoResponse> {
  return fetchJson<CoinInfoResponse>(`/api/info?id=${encodeURIComponent(String(id))}`, CACHE_TTL_MS);
}

export async function getCoinQuote(id: string | number): Promise<CoinQuoteResponse> {
  return fetchJson<CoinQuoteResponse>(`/api/quote?id=${encodeURIComponent(String(id))}`, CACHE_TTL_MS);
}
